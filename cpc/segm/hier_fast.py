...
from cpc.segm.costsegmcython import costSegm
...
def computeSEcosts(enc, maxSegmLen):
    ...
    linSums = torch.zeros((maxSegmLen, 1, 1, 1), dtype=torch.float32).cuda().repeat(1,*(enc.shape))
    sqSums = torch.zeros((maxSegmLen, 1, 1, 1), dtype=torch.float32).cuda().repeat(1,*(enc.shape))
    ...
    for i in range(1,maxSegmLen):  #Sqrt):
        ...
        linSums[i] = torch.roll(linSums[i-1], 1, 1) + enc
        sqSums[i] = torch.roll(sqSums[i-1], 1, 1) + encSq
    ...
    #print(sqSums.shape, torch.square(linSums).shape, (torch.arange(maxSegmLen)+1).view(-1,1,1,1).cuda().shape)  #, (torch.square(linSums) / torch.arange(i+1).cuda()).shape)
    costs = (sqSums - torch.square(linSums) / (torch.arange(maxSegmLen)+1).view(-1,1,1,1).cuda()).sum(-1)
...
def costSegmOld(costs, shape, maxCost, k, minSegmsInLine):
...
class FastHierarchicalSegmentationLayer(Function):
    ...
    def forward(ctx, inputGPU, maxSegmentCost, k, maxSegmLen, minSegmsPerLine=None): 
        ...
        with torch.no_grad():
            ...
            #%#t5 = time.time()
            #%#print(f"actual forward segm time: {t1-t0}, {t2-t1}, {t3-t2}, {t4-t3}, nonsense conversion times: {t51-t50}, {t52-t51}, {t5-t52} (all conversions {t5-t50}); all {t5-t0}")
    ...
    def backward(ctx, dxThrough, segm=None, ind=None, lens=None, nums0=None, nums1=None, maxl=None, shapet=None, costt=None, actualkt=None):  #, finalSegments=None, segmentNumsInLines=None):
        ...
        with torch.no_grad():
            ...
            #print("***", backprop)
            #%#t1 = time.time()
            #%#print(f"segm backward time: {t1-t0}")
...
class FastSegmentationLengthRestoreLayer(Function):
    ...
    def forward(ctx, inputGPU, numsInLinesC0, numsInLinesC1, shrinkIndices, maxInLine, targetShape): # shrinkIndices & maxInLine only for backward
        ...
        with torch.no_grad():
            ...
            #%#t2 = time.time()
            #%#print(f"len restore forward time: {t1-t0}, {t2-t1}")
            return restored
    ...
    def backward(ctx, dxThrough):  #, finalSegments=None, segmentNumsInLines=None):
        ...
        with torch.no_grad():
            ...
            #print("***-", backprop)
            #%#t1 = time.time()
            #%#print(f"len restore backward time: {t1-t0}")
            return backprop, None, None, None, None, None
